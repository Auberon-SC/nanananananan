...
from Yukki import app, BOT_NAME
...
async def close_admin(_, query: CallbackQuery):
    ...
    if not a.can_manage_voice_chats:
        return await query.answer(
            "💡 Only admin with manage video chat permission that can tap this button !",
            show_alert=True,
        )
    ...
    try:
        if userLastDownloadTime > datetime.now():
            ...
            return await CallbackQuery.message.reply_text(
                f"Hello {Name}, Please wait for `{wait_time}` min(s) before do next download request."
            )
    except:
        pass
...
async def boom(_, CallbackQuery):
    ...
    await CallbackQuery.edit_message_text(
        "📥 **Download started** !\n\n» Downloading speed could be slow, please wait for a while...",
        reply_markup=inl,
    )
    fetched = f"""
💡 **Media Downloaded.**
...
async def send_file(CallbackQuery, med, filename, videoid, user_id, link, channel):
    await CallbackQuery.edit_message_text(
        "📤 **Upload started** !\n\n» Uploading speed could be slow, please wait for a while...",
        reply_markup=upl,
    )
    try:
