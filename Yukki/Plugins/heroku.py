...
from Yukki.YukkiUtilities.helpers.heroku import is_heroku
from Yukki.YukkiUtilities.helpers.paste import paste as paste_queue
...
@app.on_message(filters.command("getlogs") & filters.user(SUDOERS))
async def log_(client, message):
    ...
    if await is_heroku():
        ...
        return await app.send_message(chat_id, "Only for Heroku Apps")
    try:
    ...
    if len(data) > 1024:
        ...
        return await app.send_message(chat_id, data)


@app.on_message(filters.command("getvars") & filters.user(SUDOERS))
async def varget_(client, message):
    ...
    if await is_heroku():
        ...
        if check_var in heroku_config:
            ...
            return await app.send_message(chat_id, "No such Var")
    else:
        path = dotenv.find_dotenv()
        if not path:
            return await app.send_message(chat_id, ".env not found.")
...
@app.on_message(filters.command("delvars") & filters.user(SUDOERS))
async def vardel_(client, message):
    ...
    if await is_heroku():
        ...
        if check_var in heroku_config:
            ...
            return await app.send_message(chat_id, f"No such Var")
    else:
        path = dotenv.find_dotenv()
        if not path:
            return await app.send_message(chat_id, ".env not found.")
...
@app.on_message(filters.command("setvars") & filters.user(SUDOERS))
async def set_var(client, message):
    ...
    if await is_heroku():
        ...
        if to_set in heroku_config:
            ...
            await app.send_message(
                chat_id, f"Added New Var with name `{to_set}`. Bot will Restart Now."
            )
...
async def usage_dynos(client, message):
    ...
    dyno = await app.send_message(chat_id, "Checking Heroku Usage.... Please Wait")
...
@app.on_message(filters.command("updates") & filters.user(SUDOERS))
async def update_(client, message):
    ...
    response = await app.send_message(chat_id, "Checking for available updates...")
    try:
        repo = Repo()
    ...
    if verification == "":
        return await response.edit("Resso Music is already using the lattest commit")
    updates = ""
    ...
    _update_response_ = "A new update is available for the Bot!</b>\n\n➣ Pushing Updates Now</code>\n\n**Updates:**\n\n"
    ...
    if await is_heroku():
        try:
            await response.edit(
                f"{nrs.text}\n\nBot was updated successfully on Heroku! Now, wait for 2 - 3 mins until the bot restart!"
            )
            os.system(
        ...
        os.system(f"kill -9 {os.getpid()} && -m Yukki")
        exit()
    return
...
async def restart_(_, message):
    chat_id = message.chat.id
    response = await app.send_message(chat_id, "Restarting....")
    if await is_heroku():
        if HEROKU_API_KEY == "" and HEROKU_APP_NAME == "":
            return await app.send_message(
                chat_id,
